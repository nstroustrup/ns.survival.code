#FROM CRAN R repository
#With Bug Fixes

#View of /pkg/survival/R/quantile.survfit.R
...
doquant <- function(p, time, surv, upper, lower) {
    findq <- function(x, y, p) {
        ...
        if (all(is.na(y)|y==1)) return(rep(NA, length(p)))
        #remove dups (censors)
        toss <- duplicated(y)
        if (any(toss)) {
            newy <- y[!toss]
        ...
        if (is.matrix(x$surv) && ncol(x$surv) >1) {
            ...
            for (strat in 1:nstrat) {
                ...
                if (conf.int) {
                    ...
                    for (i in 1:ncol(z$surv)) 
                        qmat[i,] <- doquant(probs, z$time, z$surv[,i])
                }
            }
